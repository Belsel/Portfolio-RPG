...
static void ToMergeParams(const TArray<FSkelMeshMergeSectionMapping_BP>& InSectionMappings, TArray<FSkelMeshMergeSectionMapping>& OutSectionMappings)
{
    if (InSectionMappings.Num() > 0)
    {
        ...
        {
            OutSectionMappings.AddDefaulted();
...
static void ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
    if (InUVTransformsPerMesh.Num() > 0)
    {
        ...
        FSkelMeshMergeUVTransforms TempTransforms;
        TempTransforms.UVTransformsPerMesh.AddDefaulted();
        for (int32 i = 0; i < InUVTransformsPerMesh.Num(); i++)
        {
            OutUVTransformsPerMesh.Add(TempTransforms);
        }
        UE_LOG(LogTemp, Warning, TEXT("Number of Uninitialized: %d"), OutUVTransformsPerMesh.Num());
        for (int32 i = 0; i < InUVTransformsPerMesh.Num(); ++i)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("Number of InUVTransforms: %d"), InUVTransforms.Num());
            if (InUVTransforms.Num() > 0)
            {
                OutUVTransforms.Empty();
                for (int32 k = 0; k < InUVTransforms.Num(); k++)
                {
                    OutUVTransforms.AddDefaulted();
                }
                UE_LOG(LogTemp, Warning, TEXT("Number of OutUVTransforms: %d"), OutUVTransforms.Num());
